...
public abstract class ZLTextView extends ZLTextViewBase {
    ...
    public synchronized void onScrollingFinished(PageIndex pageIndex) {
        switch (pageIndex) {
            ...
            case previous: {
                ...
                // clear page info
                myPreviousPage.reset();
                if (myCurrentPage.PaintState == PaintStateEnum.NOTHING_TO_PAINT) { //如果该页什么信息都没有，计算页面文本终止游标
    ...
    private synchronized void preparePaintInfo(ZLTextPage page) {
        //getTextColumnWidth : 列宽  可支持双列显示
        //getTextAreaHeight: 文本区域高度
        // twoColumnView: 双列显示 （横屏时候支持）
